...
const FallingText: React.FC<FallingTextProps> = ({
    ...
    const newHTML = words
      .map((word) => {
        ...
        return `<span class="inline-block mx-[3px] select-none ${
          isHighlighted ? 'text-purple-brand font-bold' : ''
        }">${word}</span>`;
      })
      .join(' ');
  ...
  useEffect(() => {
    ...
    // Only add mouse constraint on non-touch (desktop) devices to avoid blocking scroll
    ...
    if (mouseConstraint) worldBodies.push(mouseConstraint as unknown as Matter.Body);
  ...
  }, [effectStarted, gravity, wireframes, backgroundColor, mouseConstraintStiffness, prefersReducedMotion]);

  const handleTrigger = () => {
    if (!effectStarted && !prefersReducedMotion && (trigger === 'click' || trigger === 'hover')) {
      setEffectStarted(true);
    }
  };
